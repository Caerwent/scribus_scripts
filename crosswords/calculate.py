...
class Crossword(object):
    ...
    def prep_grid_words(self):
        ...
        self.available_words = [word[:3] for word in self.available_words]
        self.first_word(self.available_words[0])

    def compute_crossword(self, time_permitted=1.00):
        self.best_rows = self.rows
        self.best_cols = self.cols
        self.best_wordlist = []
    ...
    def removeRow(self, row) :
        r = row
        while r < self.best_rows-1 :
            self.best_grid[r]=self.best_grid[r+1]
            r+=1
        self.best_grid.pop()
        for word in self.best_wordlist :
            if word[IDX_ROW] > row :
                word[IDX_ROW] = word[IDX_ROW]-1
    ...
    def removeCol(self, col) :
        ...
        for r in range(self.best_rows):
            c = col
            while c < self.best_cols-1 :
                self.best_grid[r][c]=self.best_grid[r][c+1]
                c+=1
            self.best_grid[r].pop()
        for word in self.best_wordlist :
            if word[IDX_COL] > col :
                word[IDX_COL] -= 1
    ...
    def pack_grid(self):
        r = 0
        while r < self.best_rows :
            isEmptyRow = True
            c = 0
            while c < self.best_cols :
                if self.best_grid[r][c] != self.empty :
                    isEmptyRow = False
                    c = self.best_cols
                else :
                    c=c+1
